...
export async function fetchExamCentresByRegion(pageNumber: number, pageSize: number = 11, regionId: number, sortBy: string = "code", sortOrder: SortOrder = "ASC"): Promise<ApiResponse> {
    ...
    let url = `${API_URL}/exam-centres/upload-details/search?regionId=${regionId}&page=${pageNumber - 1}&size=${pageSize}&sort=${sortBy},${sortOrder}`;
...
export async function searchExamCentresWithRegion(query: string, pageNumber: number, pageSize: number = 11, regionId: number, sortBy: string = "code", sortOrder: SortOrder = "ASC"): Promise<ApiResponse> {
    const idContext = identityContext();
    if (!idContext.authenticated) {
        redirect("/login")
    }
    ...
    // page number is zero-based in backend API. So page = pageNumber - 1
    let url = `${API_URL}/exam-centres/upload-details/search?query=${encodeURIComponent(query)}&regionId=${regionId}&page=${pageNumber - 1}&size=${pageSize}&sort=${sortBy},${sortOrder}`;
    const token = idContext.token as string;
