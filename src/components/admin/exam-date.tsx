...
interface ExamDateProps {
    ...
    changeSelectedExamDate: Dispatch<SetStateAction<IExamDate>>
}

const ExamDate = ({
                      examDate,
                      index,
                      changeSelectedExamDate,
                      setShowEditModal,
                      setShowDeleteModal
                  }: ExamDateProps) => {

    const handleEditClick = async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        changeSelectedExamDate(examDate);
    ...
    return (
        <>
            <tr key={examDate.id} className="border-1 hover:bg-gray-100">
                ...
                <td className="px-6 py-4 text-center">
                    {examDate.createdDate?.toLocaleString()}
                </td>
                <td className="px-6 py-4 text-center">
