...
interface ExamCentreProps {
    examCentre: IExamCentre,
    index: number
    setShowEditModal: Dispatch<SetStateAction<boolean>>
    setShowDeleteModal: Dispatch<SetStateAction<boolean>>
    changeSelectedExamCentre: Dispatch<SetStateAction<IExamCentre>>
}

const ExamCentre = ({
                        examCentre,
                        index,
                        changeSelectedExamCentre,
                        setShowEditModal,
                        setShowDeleteModal
                    }: ExamCentreProps) => {

    const handleEditClick = async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        changeSelectedExamCentre(examCentre);
    ...
    return (
        <>
            <tr key={examCentre.id} className="border-1 hover:bg-gray-100">
                ...
                <td className="px-6 py-4 text-center">
                    {examCentre.createdDate?.toLocaleString()}
                </td>
                <td className="px-6 py-4 text-center">
                    {examCentre.modifiedDate?.toLocaleString()}
                </td>
