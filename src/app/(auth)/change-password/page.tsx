...
const ChangePasswordPage = () => {
    ...
    const [disableNewPassword, setDisableNewPassword] = useState<boolean>(true);
    const [disableConfirmPassword, setDisableConfirmPassword] = useState<boolean>(true);

    // Clear form fields when the API response state changes
    useEffect(() => {
    ...
    // Enable New Password field after Old Password is entered
    useEffect(() => {
        if (oldPassword) {
    ...
    // Validate New Password and compare with Old Password
    useEffect(() => {
        if (newPassword === oldPassword && newPassword) {
            setSamePasswordError("New password cannot be the same as the old password.");
            setDisableConfirmPassword(true);
            setDisabledBtn(true);
        } else {
        ...
        if (confirmNewPassword !== newPassword) {
            setPasswordMatchMsg("New password and confirmation password do not match.");
            setDisabledBtn(true);
        } else if (confirmNewPassword && !samePasswordError) {
            setPasswordMatchMsg("");
            setDisabledBtn(false);
        }
    }, [newPassword, confirmNewPassword, oldPassword, samePasswordError]);
                ...
                <div className="w-full bg-white rounded-lg shadow-lg md:mt-0 sm:max-w-md xl:p-0">
                    <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <div className='text-center text-red-500'>
                            <p>
                                {apiResState?.message}
                                {apiResState?.message ? "" : passwordMatchMsg || samePasswordError}
                            </p>
                        </div>
                        <form className="space-y-4 md:space-y-6" action={action}>
                            ...
                            <button type="submit" disabled={disableBtn}
                                className="w-full text-white bg-primary-700 hover:bg-primary-700 disabled:bg-primary-100 disabled:text-black focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                                Change Password
                            </button>
                        </form>
                    </div>
