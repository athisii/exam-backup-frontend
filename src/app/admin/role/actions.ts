...
import {ApiResponse, IRole, SortOrder} from "@/types/types";
...
export async function fetchRolesAsPage(pageNumber: number, pageSize: number = 8, sortBy: string = "code", sortOrder: SortOrder = "ASC"): Promise<ApiResponse> {
    ...
    let url = `${API_URL}/roles/page?page=${pageNumber - 1}&size=${pageSize}&sort=${sortBy},${sortOrder}`;
...
export async function deleteRoleById(id: number): Promise<ApiResponse> {
    ...
    let url = `${API_URL}/roles/soft-delete/${id}`;
    const token = idContext.token as string;
